tree_ws_hull.ctg <- function (las, chm, th,tol,ext, epsg = 25832)
{
  las= readLAS(las)
  if (is.empty(las)) return(NULL)
  
  #ht_chunk <- lidR::normalize_height(las_chunk, dtm)
  algo_all <- lidR::watershed(chm, th = th,tol = tol,ext = ext)
  ht_ws_chunk <- lidR::segment_trees(las, algo_all, uniqueness = "incremental")
...
tree_ws_hull = function(las, ...)
{
  UseMethod("tree_ws_hull", las)
}
